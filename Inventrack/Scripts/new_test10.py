...
class AddNewItemScreen(QDialog):
        ...
        def Enter_Data(self):
            ...
            if len(Product_ID)==0 or len(Product_Name) == 0 or len(Box) == 0 or len(Quantity) == 0 or len(Mfg_Date)==0 or len(Exp_Date)==0 or len(GRN)==0:
                self.error.setText("Please fill in all inputs.")
            else:
                user_info = [Product_ID, Product_Name, Box, Quantity, Mfg_Date, Exp_Date, GRN, Status]
            ...
            '''if len(Product_ID)==0 or len(Product_Name) == 0 or len(Box) == 0 or len(Quantity) == 0 or len(Mfg_Date)==0 or len(Exp_Date)==0 or len(GRN)==0:
                ...
                cur = conn.cursor()
                user_info = [Product_ID, Product_Name, Box, Quantity, Mfg_Date, Exp_Date, GRN, Status]
                cur.execute('INSERT INTO Store_Data (Product_ID, Product_Name, Box, Quantity, Mfg_date, Exp_date, GRN, Status) VALUES (?,?,?,?,?,?,?,?)',user_info)
                conn.commit()
                conn.close()'''
...
'''
            try:
                ...
                self.productID.setText("Not Connected")
'''
